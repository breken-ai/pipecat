...
class TestWorkerObserverInAPipeline(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_an_observer_is_told_once_however_far_a_frame_travels(self):
        observer = RecordingObserver()
        await self._run(observer)
        self.assertEqual(len(observer.pushes), 1)
        self.assertTrue(observer.pushes[0][1])

    async def test_an_observer_that_wants_every_push_gets_every_hop(self):
        observer = RecordingObserver(observe_every_push=True)
        await self._run(observer)
